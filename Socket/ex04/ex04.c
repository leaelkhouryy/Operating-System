// Write a TCP server using socket that:
// 1. Accepts a client 
// 2. Receives data sent by that client through the socket
// 3. Sends back exactly the same data to the client
// 4. Continue until the client disconnects
// 5. Properly closes all sockets before terminating
...
int main(int argc, char**argv){
    ...
    socketlen_t client_addr_len = sizeof(client_addr);
    ...
    while((n = recv(socket_connection_fd, buf, BUFSIZE, 0)) > 0){
